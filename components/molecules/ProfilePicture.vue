  ...
  <div class="profile-section">
    <div class="outsite-frame">
      <div class="preview-section">
        <img class="profile-image" style="width: 100%;height:100%" v-if="!(imageUrl && file)" :src="assets('common/temporary-profile.png')" />
        <img class="profile-image" style="width: 100%;height:100%" v-if="imageUrl && file" :src="imageUrl" />
      </div>
    </div>
    <button @click="openDropdown" v-click-outside="toggleLock">
      <img
        class="camera-btn"
        :src="assets('icon/profile-camera.png')"
      />
    </button>
    <div class="dropdown-select" v-if="isShowDropdown" v-click-outside="closeDropdown">
      ...
      <button class="second-option" style="text-align: start;" @click="deleteProfile" :class="isDisabled">
...
export default class ProfilePicture extends Vue {
  ...
  private fileSelected(event: any): void {
    ...
    if(!this.file){ //if don't select any file
      this.isShowDropdown = false
      return
    }

    else if (!regexFileType.test(this.file.type)) { // wrong type
      this.errorMessage = this.$t('profile.errorFileType').toString()
    }
    
    else if (this.file.size > 5 * 1024 * 1024) { // out of size
      this.errorMessage = this.$t('profile.errorFileSize').toString()
    }
    else { //success case
      this.file &&
